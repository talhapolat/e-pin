...
$uploadDirectory = "/images/";
    ...
    $productİnsertQuery = $dbConnect->prepare("INSERT INTO product (category_id, title, image, qty, price, stock, smalldesc, description, description2, slug,  statu) values (?,?,?,?,?,?,?,?,?,?,?)");
    $productİnsertQuery->execute([$inputcategory, $inputtitle, $fileName, $inputqty, $inputprice, $inputstock, $inputsmalldesc, $inputdescription, $inputdescription2, $slug, 1]);
...
if (array_key_exists('newcategory', $_POST)) {
  ...
  $uploadDirectory = "/images/";
    ...
    $productİnsertQuery = $dbConnect->prepare("INSERT INTO category (title, product_type, description, description2, description3, price, birim1, birim2, slug, image, in_name, in_facemail, in_facepass, in_phone, in_gameid, in_zyngavip, custompacket, statu) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
    $productİnsertQuery->execute([$inputtitle, $inputproducttype, $inputdescription, $inputdescription2, $inputdescription3, $inputprice, $inputbirim1, $inputbirim2, $slug, $fileName, $_POST["checkInName"], $_POST["checkInFace"], $_POST["checkInFacePass"], $_POST["checkInPhone"], $_POST["checkInGameId"], $_POST["checkInZyngaVip"], $_POST["custompacket"], 1]);
    $insertControl = $productİnsertQuery->rowCount();
...
if (array_key_exists('newblog', $_POST)) {
  ...
  $blogİnsertQuery = $dbConnect->prepare("INSERT INTO blogs (title, text, topic, slug, statu, created_at) values (?,?,?,?,?,?)");
  $blogİnsertQuery->execute([$inputtitle, $inputtext, $inputtopic, $slug, 1, $time]);
...
if (array_key_exists('updatemainsettings', $_POST)) {
  ...
  $settingİnsertQuery = $dbConnect->prepare("UPDATE settings SET
    ...
    WHERE id = ?");
  $settingİnsertQuery->execute([$inputtitle, $inputurl, $inputdescription, $inputkeywords, $inputaddress, $inputphone, $inputemail, $fileNameLogo, $fileNameAlt, $setting["id"]]);
...
if (array_key_exists('newslider', $_POST)) {
  ...
  $uploadDirectory = "/images/slider/";
    ...
    $sliderİnsertQuery = $dbConnect->prepare("INSERT INTO slider (title, image, link, number, statu) values (?,?,?,?,?)");
    $sliderİnsertQuery->execute([$inputtitle, $fileName, $inputlink, $number+1, 1]);
  ...
  $sliderİnsertQuery = $dbConnect->prepare("UPDATE slider SET
    ...
    WHERE id = ?");
  $sliderİnsertQuery->execute([$inputtitle, $fileName, $inputlink, $slider["statu"], $slider["id"]]);
  $insertControl = $sliderİnsertQuery->rowCount();
